...
bool Cartridge::load(const uint8_t* data, size_t size) {
    // Minimum size check (header + some data)
    if (size < sizeof(iNESHeader)) {
    ...
    std::memcpy(m_prg_rom.data(), data + offset, prg_size);
    offset += prg_size;

    // Copy CHR ROM (or allocate CHR RAM)
    if (chr_size > 0) {
        m_chr_rom.resize(chr_size);
        std::memcpy(m_chr_rom.data(), data + offset, chr_size);
        m_has_chr_ram = false;
    } else {
        // No CHR ROM means CHR RAM (8KB)
        m_chr_rom.resize(8192, 0);
        m_has_chr_ram = true;
    }
    ...
    // Determine mirror mode from header
    MirrorMode mirror;
    ...
    // Create mapper
    m_mapper.reset(create_mapper(m_mapper_number, m_prg_rom, m_chr_rom, m_prg_ram,
                                 mirror, m_has_chr_ram));
    if (!m_mapper) {
        std::cerr << "Failed to create mapper " << m_mapper_number << std::endl;
        return false;
    }
    ...
    std::cout << "  Mapper: " << m_mapper_number << std::endl;
    std::cout << "  PRG ROM: " << prg_size / 1024 << " KB" << std::endl;
    std::cout << "  CHR " << (m_has_chr_ram ? "RAM" : "ROM") << ": "
              << m_chr_rom.size() / 1024 << " KB" << std::endl;
...
bool Cartridge::parse_header(const iNESHeader& header) {
    // Extract mapper number
    m_mapper_number = (header.flags6 >> 4) | (header.flags7 & 0xF0);

    // Check for NES 2.0 format
    if ((header.flags7 & 0x0C) == 0x08) {
        // NES 2.0 - extended mapper number
        m_mapper_number |= (header.flags8 & 0x0F) << 8;
    }

    // Flags
    m_has_battery = (header.flags6 & 0x02) != 0;
    m_has_trainer = (header.flags6 & 0x04) != 0;
...
void Cartridge::cpu_write(uint16_t address, uint8_t value) {
    if (m_mapper) {
        m_mapper->cpu_write(address, value);
    }
}
...
void Cartridge::ppu_write(uint16_t address, uint8_t value) {
    if (m_mapper) {
        m_mapper->ppu_write(address, value);
    }
}

MirrorMode Cartridge::get_mirror_mode() const {
    if (m_mapper) {
        return m_mapper->get_mirror_mode();
    }
...
void Cartridge::save_state(std::vector<uint8_t>& data) {
    // Save PRG RAM (may include battery-backed SRAM)
    write_value(data, static_cast<uint32_t>(m_prg_ram.size()));
    if (!m_prg_ram.empty()) {
...
void Cartridge::load_state(const uint8_t*& data, size_t& remaining) {
    ...
    // Load mapper state
    if (m_mapper) {
        m_mapper->load_state(data, remaining);
    }
}
